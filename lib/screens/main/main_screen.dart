...
class MainScreenState extends State<MainScreen> {
  ...
  get user => _user;
  set user(value) => setState(() => _user = value);

  get todos => _todos;
  set todos(value) => setState(() => _todos = value);

  get isBusy => _isBusy;
  set isBusy(value) => setState(() => _isBusy = value);

  void populateData(User loginUser) async {
    isBusy = true;
    user = loginUser;
    todos = await TodoService.getTodoListByUser(loginUser.id);
    isBusy = false;
  }
  ...
  void removeTodo(int index) async {
    await TodoService.removeTodo(_todos[index]);
    populateData(user);
  }
